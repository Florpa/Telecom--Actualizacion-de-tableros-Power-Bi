...
cur.execute('SELECT  max (semana)FROM tablero_regeneracion_urbana.data_auxiliar')
...
cur.execute("UPDATE tablero_regeneracion_urbana.data_auxiliar a SET find_anterior = b.semana, franja_findeante= '1-Mediodia',dispositivos_findeanter= b.n_lineas from (select fraccion, a.semana, round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana IN (5,6,7,8) AND a.semana = "+str(semana_anterior)+"and  hora between '11:00' and '15:00' group by fraccion,a.semana)b where a.id_cuadricula= b.fraccion  and dispositivos_findeanter is null and a.semana = "+str(semana_actual))
...
cur.execute("INSERT INTO tablero_regeneracion_urbana.data_auxiliar (id_cuadricula, semana,franja,  dispositivos_semana)select fraccion, a.semana, '2-Tarde',round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha)where c.dia_semana NOT IN (5,6,7,8) AND a.semana = "+str(semana_actual)+" and hora between '15:30' and '17:00'   group by fraccion,a.semana;")
connection.commit()
...
cur.execute("UPDATE tablero_regeneracion_urbana.data_auxiliar a SET find_anterior = b.semana, franja_findeante= '2-Tarde',dispositivos_findeanter= b.n_lineas from (select fraccion, a.semana, round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana IN (5,6,7,8) AND a.semana = "+str(semana_anterior)+"and  hora between '15:30' and '17:00' group by fraccion,a.semana)b where a.id_cuadricula= b.fraccion  and dispositivos_findeanter is null and a.semana = "+str(semana_actual))
connection.commit()
...
cur.execute("INSERT INTO tablero_regeneracion_urbana.data_auxiliar (id_cuadricula, semana,franja,  dispositivos_semana)select fraccion, a.semana, '3-Noche',round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha)where c.dia_semana NOT IN (5,6,7,8) AND a.semana = "+str(semana_actual)+" and hora between '17:30' and '23:00'   group by fraccion,a.semana;")
connection.commit()
...
cur.execute("UPDATE tablero_regeneracion_urbana.data_auxiliar a SET poblacion = n_lineas from (select a.semana, fraccion,a.semana as sem,round(median(a.moviles::int))as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana not IN (5,6,7,8) and a.semana= "+str(semana_actual)+" and hora = '4:00' group by a.semana,fraccion) b where a.id_cuadricula= b.fraccion and poblacion is null and a.semana= "+str(semana_actual))
connection.commit()
...
cur.execute("update	tablero_regeneracion_urbana.data_auxiliar a set poblacion_pre= b.n_lineas from (select fraccion, a.semana, round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana  IN (5,6,7,8) and hora = '4:00' and a.semana=2 group by fraccion,a.semana)b where a.id_cuadricula= b.fraccion and poblacion_pre is null and a.semana= "+str(semana_actual))
connection.commit()
print ('TERMINE DE ACTUALIZAR LA POBLACION DE ESA SEMANA')
...
cur.execute("update tablero_regeneracion_urbana.data_auxiliar a set semanprecu =b.semana, franja_pre= '1-Mediodia',dispositivo_semaprec= n_lineas from (select fraccion, a.semana,  round(median (a.moviles::int)) as n_lineas from datos.dispositivos_por_grilla a left  join aux.calendario_2020 c using (fecha) where c.dia_semana NOT IN (5,6,7,8) and hora between '11:00' and '15:00' and a.semana =2 group by fraccion,a.semana) b where a.id_cuadricula=b.fraccion and  a.franja= '1-Mediodia'and a.semana= "+str(semana_actual))
connection.commit()
...
cur.execute("update tablero_regeneracion_urbana.data_auxiliar  a set actualizacion ='"+actualizacion+"',descripcion='"+periodo+"' where semana="+str(semana_actual))
connection.commit()
print(datetime.datetime.today())
